...
final readonly class CsvImportService
{
    ...
    public function errorReport(array $errors): string { $stream = fopen('php://temp', 'r+'); if ($stream === false) { throw new \RuntimeException('CSV stream unavailable.'); } fputcsv($stream, ['Zeile', 'Fehler'], ';', '"', ''); foreach ($errors as $error) { fputcsv($stream, [$error['line'], $error['message']], ';', '"', ''); } rewind($stream); $csv = stream_get_contents($stream); fclose($stream); return $csv; }
}

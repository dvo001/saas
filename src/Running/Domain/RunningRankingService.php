...
final readonly class RunningRankingService
{
     ...
     */
    public function qualification(array $rows):array { $rankable=[];foreach($rows as $row){$times=$row['times'];sort($times,SORT_NUMERIC);if($times!==[]){$row['times']=$times;$rankable[]=$row;}}usort($rankable,static fn(array $a,array $b):int=>self::compareTimeVectors($a['times'],$b['times']));$rank=0;$previous=null;foreach($rankable as $index=>&$row){if($previous===null||$row['times']!==$previous){$rank=$index+1;$previous=$row['times'];}$row['rank']=$rank;}unset($row);return $rankable; }
    /**
     ...
     */
    private static function compareTimeVectors(array $left,array $right):int{$length=max(count($left),count($right));for($i=0;$i<$length;++$i){$comparison=($left[$i]??PHP_INT_MAX)<=>($right[$i]??PHP_INT_MAX);if($comparison!==0){return $comparison;}}return 0;}
}
